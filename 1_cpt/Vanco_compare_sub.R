...
# One-compartment model with linear elimination (IV)
# Parameters based on median values from neonatal population literature

## ----- Loading library ----
library(pacman)
pacman::p_load(admr, rxode2, tidyverse, MASS, data.table, pracma, Matrix,
...
# Values adapted for a 1 kg neonate using median literature values
TRUE_PARAMS <- list(
  theta = c(cl = 0.06, v = 0.58),      # Median typical values: CL = 0.06 L/kg/h, V = 0.58 L/kg
  iiv_sd = c(cl = 0.229, v = 0.21),    # Median IIV: CL = 22.9%, V = 21.0%
  sigma_prop = 0.117,                  # Median Proportional RV = 11.7%
  sigma_add = 0.2,                     # Median Additive RV = 0.2 mg/L
  dose = 15                            # Assuming 15 mg/kg dose for 1kg neonate
)

TRUE_OMEGA <- diag(TRUE_PARAMS$iiv_sd^2)

# Compile Model (1-Compartment)
rxModel <- rxode2({
  ke = cl / v
...
# Extract results function (Updated for 1-comp)
  ...
  tryCatch({
    ...
    base_opts <- list(
      ...
      p = list(beta = TRUE_PARAMS$theta, Omega = TRUE_OMEGA,
               Sigma_prop = TRUE_PARAMS$sigma_prop^2, Sigma_add = TRUE_PARAMS$sigma_add^2),
      nsim = 2000,
      n = n_sub,
      fo_appr = FALSE,
      omega_expansion = 1,
...
output_file <- "Vanco_Comp_Results.xlsx"
write_xlsx(list(
...
p_Vanco_50 <- ggplot(plot_data, aes(x = N_Label, y = Relative_Bias, fill = Method)) +
  ...
  labs(
    title = "Estimation Bias: Neonatal Vancomycin (1-Comp)",
...
p_Vanco_50_shaded <- ggplot(summary_plot_data, aes(x = N_Subject, y = Mean_Bias, color = Method, fill = Method)) +
  ...
  labs(
    title = "Mean Bias Convergence (Neonatal Vancomycin)",
    subtitle = "Solid lines: Mean Relative Bias | Shaded areas: ±1 Standard Deviation",
    x = "Sample Size (N)",
    y = "Relative Bias (%)",
